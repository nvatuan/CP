...
bool reroute(int K, vector<int> Visited, vector<int> &Gate_assign, vector<int> &Key_assign, const vector<vector<int>> &edgeList, int depth = 0) {
    ...
    // cerr << "\n\t Key\tGate\n";
    for(int i = 1; i <= M; i++) {
        // cerr << i << "\t\t" << (Key_assign[i] == -1 ? -1 : edgeList[i][Key_assign[i]]) << endl;
    }

    for(int idx = 1 /*Key_assign[K]*/; idx < edgeList[K].size(); idx++) {
        const int &g = edgeList[K][idx];
        
        // cerr << "\n Checking if " << g << " is available..\n";

        if( not Visited[g] ) {
            if( Gate_assign[g] == -1) {
                // cerr << " this gate was not assigned. Assigned!\n";

                Gate_assign[g]  = K;
                Key_assign[K]   = idx;
                return 1;
            } else {
                // cerr << " this gate was assigned to key = " << Gate_assign[g];
...
bool maximal_matching_exist(vector< pair<int, pair<int, int>> >::iterator L, vector< pair<int, pair<int, int>> >::iterator R) {
    ...
    for(auto it = L; it != R; it++) {
        ...
        // cerr << "Found pair: " << keyID << " : " << gateID << endl;
        // -- append to List;
    ...
    for(int k = 1; k <= M; k++) {
        if(Key_assign[k] == -1) {
            ...
            // cerr << "Set gate for key = " << k << endl;

            for(Key_assign[k] = 1; Key_assign[k] < edgeList[k].size(); Key_assign[k]++) {

                // cerr << " Key " << k << " : " << edgeList[k][Key_assign[k]] << endl;
...
int treasurePath(vector<string> islandMap) {
    ...
    if( Arrival.size() != M*M ) return -1;
        ...
        for(auto it = Arrival.begin(); it != Arrival.end(); it++) {
            // cerr << "\nExaming:\n";
            // cerr << "Time: " << it->first << " | Key: " << (it->second).first << " | Gate : " << (it->second).second << endl;
            int exitcode = maximal_matching_exist(Arrival.begin(), next(it));
            if( exitcode ) return it->first;
            // // cerr << "\nTask failed successfully.\n";
            // cerr << "Code = " << exitcode << endl;
        }
        // -- Not found.
        return -1;
    }
